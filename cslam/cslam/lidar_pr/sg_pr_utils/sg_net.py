...
class SGTrainer(object):
    ...
    def setup_model(self,train=True):
        ...
        if (not train) and self.model_pth != "":
            ...
            state_dict = torch.load(self.model_pth, map_location= 'cpu')# 'cuda:'+str(self.args.gpu)) #'cuda:0'
    ...
    def eval_feature_pair(self, features_1, features_2):
        """
        Eval on the trained net using .
        :param features_1: Torch tensor (1, _, _) with graph features for first in pair
    ...
    #     batch_feature_2 = []
    #     for graph_pair in batch:
    #         data = self.prosess_for_torch(graph_pair, False)
    #         batch_feature_1.append(data["features_1"])
    #         batch_feature_2.append(data["features_2"])
    #         target = data["target"]
    #         batch_target.append(target)
